...
$key=file_get_contents('mykey.txt');
...
$playtoken='http://8tracks.com/sets/new.json?api_key='. $key;
...
$playurl= 'http://8tracks.com/sets/'.$token.'/play.json?mix_id='.$playlistid . '?api_key='.$key;
...
else
{
    echo '<script type="text/javascript">
        function() {
            alert("Playlist done downloading to '. getcwd() .'".);
        }()
    </script>';
}  
...
$songPath = 'songs/'.$obj['set']['track']['name'].'.m4a';
file_put_contents(str_replace('\'', '', $songPath),$songfile);
}
//GET NEXT SONG
$playurl= 'http://8tracks.com/sets/'.$token.'/next.json?mix_id='.$playlistid.'?api_key='.$key;
$songcurl = curl_init($playurl);
curl_setopt($songcurl, CURLOPT_URL,$playurl);
curl_setopt($songcurl, CURLOPT_RETURNTRANSFER, true);
$songdata = curl_exec($songcurl);
